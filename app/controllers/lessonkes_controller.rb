class LessonkesController < ApplicationController
    ...
    def index
        ...
        @all_ranks = Lessonke.find(Like.group(:lessonke_id).order('count(lessonke_id) desc').limit(50).distinct.pluck(:lessonke_id))
    end
    def new
    ...
    def show
        ...
        @like = Like.new
        @lessonkes = Lessonke.where(title:@lessonke.title).order("created_at ASC") 
        @all_ranks = Lessonke.find(Like.group(:lessonke_id).order('count(lessonke_id) desc').limit(50).distinct.pluck(:lessonke_id))
    end
    def ranking
