class LessonkyosController < ApplicationController
    ...
    def index
        ...
        @all_ranks = Lessonkyo.find(Like.group(:lessonkyo_id).order('count(lessonkyo_id) desc').limit(50).distinct.pluck(:lessonkyo_id))
    end
    def new
    ...
    def show
        ...
        @like = Like.new
        @lessonkyos = Lessonkyo.where(title:@lessonkyo.title).order("created_at ASC") 
        @all_ranks = Lessonkyo.find(Like.group(:lessonkyo_id).order('count(lessonkyo_id) desc').limit(50).distinct.pluck(:lessonkyo_id))
    end
    def ranking
