class LessonbsController < ApplicationController
    ...
    def index
        ...
        @all_ranks = Lessonb.find(Likeb.group(:lessonb_id).order('count(lessonb_id) desc').limit(50).pluck(:lessonb_id))
    end
    def new
        @lessonb=Lessonb.new
    ...
    def show
        ...
        @like = Likeb.new
        @lessonbs = Lessonb.where(title:@lessonb.title,teacher_name:@lessonb.teacher_name).order("created_at ASC") 
        @all_ranks = Lessonb.find(Likeb.group(:lessonb_id).order('count(lessonb_id) desc').limit(50).pluck(:lessonb_id))
    end
    def ranking
